...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="format-day">
		...
		<xsl:param name="suffix">
			<xsl:choose>
				<xsl:when test="(substring($day,2) = 1) and not(substring($day,1,1) = 1)">ste</xsl:when>
				<xsl:when test="(substring($day,2) = 2) and not(substring($day,1,1) = 1)">de</xsl:when>
				<xsl:when test="(substring($day,2) = 3) and not(substring($day,1,1) = 1)">de</xsl:when>
	...
	<xsl:template name="format-weekday">
		...
		<xsl:choose>
			...
			<xsl:when test="$weekday = 7">Zondag</xsl:when>
		</xsl:choose>
		</xsl:param>
		<xsl:choose>
